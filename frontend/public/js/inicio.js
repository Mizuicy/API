...
function solicitarEmprestimo(id, titulo) {
    ...
    .then(async res => {
        ...
        fecharModal();
        if (res.ok) {
            mostrarNotif(`Empréstimo de "${titulo}" solicitado! Você tem 14 dias para devolver.`, 'success');
        } else {
            mostrarNotif(data.error || 'Não foi possível realizar o empréstimo.', 'error');
        }
    })
    .catch(() => {
        fecharModal();
